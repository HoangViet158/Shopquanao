...
class goodsReceipt_Model{
    ...
    public function addGoodReceiptRow($MaTK, $MaNCC, $TongTien) {
        // Sửa lại câu SQL, thiếu dấu ngoặc đơn và giá trị TrangThai
        $sql = "INSERT INTO phieunhap (MaTK, MaNCC, ThanhToan, ThoiGian, TrangThai) 
                VALUES (?, ?, ?, NOW(), 1)"; // Sửa dấu ngoặc đơn và thêm giá trị TrangThai
        
        $stmt = $this->database->prepare($sql);
        $stmt->bind_param("iid", $MaTK, $MaNCC, $TongTien);
        if ($stmt->execute()) {
            return $stmt->insert_id; // Sửa insert_id() thành insert_id
        }
        return false;
    }
